...
const Board = ({ setShowNewTaskModal, boardInfo, projectId, setShowDetailModal, index }) => {
            ...
            <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                <div className='py-3 px-4 m-auto mb-4 md:m-0 md:display-inline rounded-lg shadow-lg w-80 h-fit bg-gray-200'>
                    <div className='flex flex-col'>
                            ...
                            {isTitleEdit
                                ...
                                <form onSubmit={handleBoardTitleSubmit} className="flex items-center w-full">
                                    <input
                                        ...
                                        value={boardTitle}
                                    />
                                    <svg onClick={() => setIsTitleEdit(false)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8 ml-1 text-gray-700">
                                ...
                                : <>
                                    <h2 className='font-bold text-xl md:text-2xl ml-1'>{boardInfo.boardTitle}</h2>
                                    <div className='flex gap-2'>
                                        <svg onClick={() => setIsTitleEdit(!isTitleEdit)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 md:w-6 md:h-6 text-gray-700 hover:text-blue-500">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                        </svg>                            
                                        <svg onClick={() => deleteAlert(boardInfo.boardTitle, boardInfo._id)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 md:w-6 md:h-6 text-gray-700 hover:text-blue-500">
                        ...
                        <Droppable droppableId={boardInfo._id} >
                            {provided => 
                                <div {...provided.droppableProps} ref={provided.innerRef}>
                                    {boardInfo.tasks.length > 0 && 
                                        boardInfo.tasks.map((eachTask, index) => (
                                            <TaskCard
                                                key={eachTask._id}
                                                id={eachTask._id}
                                                index={index}
                        ...
                        <div className='flex justify-center mt-7'>
                            <button onClick={() => handleNewTask(boardInfo._id)} type='button' className='flex w-1/2 items-center justify-center inline-block p-1 border-2 border-gray-400 text-gray-500 font-medium text-xs md:text-base leading-tight uppercase rounded hover:bg-black hover:bg-opacity-5 focus:outline-none focus:ring-0 transition duration-150 ease-in-out'>
